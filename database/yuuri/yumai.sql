...
/*CREATE TABLE chatbot.user_favorites (
    id INT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
...
CREATE TABLE chatbot.tickets (
    ...
    /*derived*/
    visit_frequency_score DECIMAL(3,2),        
...
-- 무결성 점검 쿼리 예시 (배치 점검)
-- 주기적으로 "존재하지 않는 session_id"를 찾는 쿼리를 실행
/*
...
CREATE TABLE ml_features.user_interactions (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    user_id INT NOT NULL REFERENCES chatbot.users(id),
    session_id UUID REFERENCES chatbot.conversations(session_id) ON DELETE CASCADE,
