...
def run_chat_mode(config, logger):
    ...
    # NLU 모델 정보 로깅
    nlu_type = getattr(config, 'nlu_type', 'ax_lite')
    use_trained = getattr(config, 'use_trained', False)
    logger.info(f"NLU 모델: {nlu_type} (학습된 모델: {use_trained})")
    ...
    try:
        ...
        # NLU 모델 선택 적용
        if hasattr(config, 'nlu_type'):
            print(f"NLU 모델: {config.nlu_type}")
            if config.nlu_type == 'ax_encoder':
                print(f"  - 경량 인코더 모델 (600MB)")
                print(f"  - {'학습된' if config.use_trained else '학습 안된'} 모델 사용")
            else:
                print(f"  - LLM 기반 모델 (6GB)")
                print(f"  - 4비트 양자화: {getattr(config, 'use_4bit', True)}")
        
        # 챗봇 생성 시 NLU 옵션 전달
        nlu_override = None
        if hasattr(config, 'nlu_type'):
            nlu_override = {
                'nlu_type': config.nlu_type,
                'use_trained': getattr(config, 'use_trained', False),
                'use_4bit': getattr(config, 'use_4bit', True)
            }
        
        chatbot = create_naviyam_chatbot(config, nlu_override=nlu_override)
