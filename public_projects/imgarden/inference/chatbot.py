...
class ConversationMemory:
    ...
    def add_conversation(self, user_id: str, user_input: str, bot_response: str, extracted_info: ExtractedInfo, emotion: str = "happy"):
        """대화 추가"""
...
class NaviyamChatbot:
    ...
    def __init__(self, config):
        ...
        self.emotion_detector = EmotionDetector()  # 감정 검출기
        ...
        # 캐시 버전 관리 - 코드 변경 시 자동 캐시 무효화
        self.cache_version = "v2.1"  # 버전 변경시 캐시 자동 초기화
    ...
    def process_user_input(self, user_input: UserInput) -> ChatbotOutput:
        ...
        try:
            ...
            emotion = self._determine_emotion(
                extracted_info, user_input.text, response.text, user_input.user_id
            )
            ...
            if self.config.inference.save_conversations:
                self.conversation_memory.add_conversation(
                    user_input.user_id, user_input.text, response.text, extracted_info, emotion
                )
    ...
    def _perform_rag_search(self, user_input: UserInput, extracted_info: ExtractedInfo) -> str:
        ...
        recommendation_intents = [
            ...
            IntentType.BUDGET_INQUIRY,  # 예산 관련 문의
            IntentType.GENERAL_CHAT  # 일반 대화도 포함 (음식 관련일 수 있음)
        ]
        ...
        try:
            ...
            if extracted_info.entities:
                ...
                if extracted_info.entities.food_type:
                    query_parts.append(extracted_info.entities.food_type)
                
                # 예산이 있으면 추가
                if extracted_info.entities.budget:
                    query_parts.append(f"{extracted_info.entities.budget}원")
                
                # 위치가 있으면 추가
                if extracted_info.entities.location_preference:
                    query_parts.append(extracted_info.entities.location_preference)
                
                # 엔티티가 있으면 원본 텍스트와 결합
                if query_parts:
                    search_query = f"{user_input.text} {' '.join(query_parts)}"
                    logger.debug(f"개선된 RAG 검색 쿼리: {search_query}")
            
            # RAG 검색 수행
            rag_context = self.retriever.get_context_for_llm(search_query)
    ...
    def _determine_emotion(
        ...
        """감정 상태 결정"""
        try:
            # 대화 컨텍스트 수집 (이전 감정 상태 포함)
            recent_conversations = self.conversation_memory.get_recent_conversations(user_id, 3)
            
            # 컨텍스트 정보 구성
            context = {
                "is_first_interaction": len(recent_conversations) == 0,
                "previous_emotions": [conv.get("emotion", "happy") for conv in recent_conversations],
                "low_balance": False,  # 잔액 부족 상황 (향후 급식카드 연동)
                "no_recommendations": len(response_text) < 50 and "죄송" in response_text
            }
            
            # 이전 감정 상태 확인 (감정 전이 확률 적용)
            if context["previous_emotions"]:
                previous_emotion = context["previous_emotions"][-1]
            else:
                previous_emotion = "happy"
            
            # EmotionDetector로 기본 감정 결정
            ...
            # 감정 전이 확률을 고려한 최종 감정 결정
            if previous_emotion != "happy":  # 기본값이 아닌 경우만 전이 고려
                transition_probs = self.emotion_detector.get_emotion_transitions(previous_emotion)
                
                # 현재 감정이 자연스러운 전이인지 확인
                current_prob = transition_probs.get(emotion, 0.0)
                
                # 전이 확률이 너무 낮으면 (0.1 이하) 이전 감정 유지
                if current_prob <= 0.1 and confidence < 0.8:
                    emotion = previous_emotion
                    logger.debug(f"감정 전이 확률이 낮아 이전 감정 유지: {previous_emotion} -> {emotion}")
            
            logger.debug(f"감정 결정 완료: {emotion} (신뢰도: {confidence:.2f})")
            return emotion
            
        except Exception as e:
            logger.error(f"감정 결정 실패: {e}")
            # 오류 시 기본 감정 반환
            return "happy"
...
# 편의 함수들
def create_naviyam_chatbot(config, nlu_override: Optional[dict] = None) -> NaviyamChatbot:
    """나비얌 챗봇 생성 (편의 함수)
    ...
    if nlu_override:
        ...
        chatbot.nlu = NLUFactory.create_nlu_model(
        ...
        logger.info(f"NLU 모델 오버라이드: {nlu_override}")
